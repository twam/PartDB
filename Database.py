...
class Database:
    ...
    def __init__(self, filename):
        ...
        self.persistentDict = PersistentDict.PersistentDict(self.filename, format='json')
    ...
    def query(self, orderBy = 'manufacturerPartNumber', filter = None, addDefaults = True):
        filteredDict = {k: self.addDefaults(copy.copy(v)) if addDefaults else copy.copy(v) for k, v in self.persistentDict.items() if (filter(k) if filter != None else True)}
